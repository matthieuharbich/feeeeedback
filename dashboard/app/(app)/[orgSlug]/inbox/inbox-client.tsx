...
function buildJsonExport(comments: Comment[]) {
  ...
  return {
    ...
    prompt: [
      ...
      "",
      "1. **Cluster the items first.** Group feedback that share the same underlying concern (same component, same page section, same bug, same UX flow). A cluster can be a single item — that's fine. Each cluster becomes one GitHub issue.",
      "",
      "2. **Open one GitHub issue per cluster.** The issue must read like a clean spec:",
      "   - Title: short, verb-first, scoped — describes the umbrella concern in one line.",
      "     Examples: `Align Submit button to grid on mobile checkout`, `Tighten inbox selection-bar UX`.",
      "   - Body, in this exact order:",
      "     - **Summary** — one or two sentences framing the umbrella concern (only if the cluster has >1 item; for single-item issues, skip).",
      "     - **Items** — for each feedback in the cluster, a `### #<index> — <one-line>` subsection containing:",
      "       - **Feedback** — quote the `feedback` field verbatim, attribute to `from` if present.",
      "       - **Action** — quote the `action` field verbatim.",
      "       - **Context** — `page.url`, `element.selector`, `element.tag`/`element.text` when relevant.",
      "       - **Screenshots** — embed every URL from `feedbackAttachments` and `actionAttachments` as `![](url)` so they render inline. Omit nothing.",
      "       - **Source** — `feeeeedback item #<index>`, captured `createdAt`.",
      "   - No fluff, no preamble, no \"I'll now…\". Pure spec.",
      "",
      "3. **Implement** the items.",
      "",
      "4. **Commit ONCE per feedback item**, always. Never bundle multiple items into the same commit, even if they share an issue.",
      "   - Verb-first, scoped, super explicit subject describing exactly what this single item changes. Examples:",
      ...
      "   - **Link the commit to its parent issue at the end of the commit body**:",
      "     - If the issue has >1 item (cluster): use `Refs #<issue-number>` — GitHub auto-links the commit to the issue without closing it.",
      "     - If the issue has exactly 1 item: use `Closes #<issue-number>` — auto-link + auto-close on merge.",
      "     - For a cluster, the **last** commit that completes all items in the issue should use `Closes #<issue-number>` instead of `Refs`, so the issue closes when its final piece lands.",
      "   - One commit per item keeps every implementation isolated, reviewable, and revertible — while the issue gives the human-readable umbrella.",
      "",
